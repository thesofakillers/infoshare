...
class UDDataModule(LightningDataModule):
    ...
    def _map_wordpieces(self, tokens: List[List[str]]) -> List[List[int]]:
        ...
        def count_tokens(batch_idx: int) -> int:
            return sum(x != self.hparams.pad_token for x in encoding.tokens(batch_idx)) - 2  # -2 for [CLS] and [SEP]
        ...
        return [
            [-1]  # [CLS]
            + [encoding.token_to_word(b_idx, token_idx) for token_idx in range(1, count_tokens(b_idx) + 1)]  # tokens
    ...
    def prepare_data(self):
        ...
        dataset = dataset.map(self._add_pos_per_wordpiece, batched=True, remove_columns=columns)
    ...
    def train_dataloader(self) -> DataLoader:
        return DataLoader(self.ud_train,
    ...
    def test_dataloader(self) -> DataLoader:
        return DataLoader(self.ud_test,
                          batch_size=self.hparams.batch_size,
                          num_workers=self.hparams.num_workers,
                          collate_fn=self.collate_fn,
                          )
